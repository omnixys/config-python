...
> Repository development instructions for **config** (Shared Package).
...
- Repository path: `packages/python/config` (relative to the Omnixys root)
...
src/config/ pydantic-settings based configuration
...
Configuration schemas are public contracts for services.
