<!-- repository: packages/python/config | kind: PACKAGE | stack: python-package -->

# config — Skill: Package Development

> Workflow for config (packages/python/config). Execute this workflow before, during, and
> after changes in this repository.

## Repository Facts

- Kind: Shared Package
- Package: `omnixys-config` (version: dynamic)
- Runtime: Python >=3.14 (uv)
- Description: Omnixys unified configuration package (pydantic-settings).
- Architecture: src/config/ pydantic-settings based configuration
...
- This package is published (`omnixys-config`); consumers pin versions and rely on SemVer.
